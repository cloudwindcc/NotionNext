...
const BLOG = {
  ...
  NEXT_REVALIDATE_SECOND: process.env.NEXT_PUBLIC_REVALIDATE_SECOND || 86400, // 更新缓存间隔 单位(秒)；默认每个页面有1天的纯静态期，降低ISR频繁重建导致的Vercel Function消耗。
  NOTION_API_TIMEOUT_MS: process.env.NOTION_API_TIMEOUT_MS || 8000, // Notion接口超时时间，避免函数一直等待到Vercel Timeout。
  NOTION_API_RETRY_ATTEMPTS: process.env.NOTION_API_RETRY_ATTEMPTS || 1, // Notion接口失败重试次数，生产环境默认快速失败以保护函数时长。
