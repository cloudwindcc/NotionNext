...
const notionApiTimeoutMs = parsePositiveInt(BLOG.NOTION_API_TIMEOUT_MS, 8000)
...
function withTimeout(promise, label) {
  ...
  const timeout = new Promise((_, reject) => {
    timer = setTimeout(() => {
      reject(new Error(`${label} timed out after ${notionApiTimeoutMs}ms`))
    }, notionApiTimeoutMs)
  })
