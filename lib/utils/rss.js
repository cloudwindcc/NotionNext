...
export async function generateRss(props) {
  ...
  try {
    writeGeneratedRssFile('feed.xml', feed.rss2())
    writeGeneratedRssFile('atom.xml', feed.atom1())
    writeGeneratedRssFile('feed.json', feed.json1())
  } catch (error) {
...
function writeGeneratedRssFile(file, content) {
  for (const directory of ['./public/rss', './out/rss']) {
